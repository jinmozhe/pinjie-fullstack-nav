...
站点可在 Admin 和管理 API 中暂不设置分类，便于批量插入后补全；未分类站点不能发布或置顶，批量发布、置顶及单条保存为已发布或置顶都会被服务端拒绝。Web 只显示已发布、未删除且分类启用的站点。分组和完整列表共用可见条件，分类按 `sort_order` 升序、ID 倒序稳定排序，站点按 `sort_order` 升序、ID 倒序稳定排序；新建站点默认 `sort_order=1,000,000`，历史数据不因本次规则调整而改写。数据库窗口查询先计算分类总量与站点序号，再截取每组预览，不逐分类发起无界请求。站点下架、分类停用或站点删除使凭据查阅接口同时不可见；Admin 仍可读取已保留资料。恢复站点后保持未发布。图标使用 `navigation_icon` 上传场景，限制 2 MB 的 PNG/JPEG/WebP，仍被任何站点引用时不能删除资产。
...
- Web 关闭详情、会话失效或退出时隐藏帐号并移除查询缓存；同源标签通过 BroadcastChannel 同步 Web 退出。
- Web SSR 只读取并透传 reader Session Cookie，校验成功后请求查阅分类和站点。缺少会话或明确 401 使用公开入口；403、网络及服务故障展示加载失败，不把故障当作认证成功。
- 分类、分组、站点列表和详情使用独立的公开与管理员标识查询缓存，查阅数据只存内存。身份重新核验、退出请求期间隐藏授权内容；退出、身份失效取消请求并清除查阅缓存，禁止旧响应恢复内容。身份失效时回到全部站点第一页；身份有效但当前筛选不再可见时显示错误。分类、分组、站点和详情不会定时刷新，也不会因切回浏览器 Tab 自动重新请求；用户主动重试或切换列表条件时才重新读取。
...
`/x` 的列表由 `x-navigation` Feature 维护，通过其公开入口提供展示与服务端读取。路由调用 Next.js `connection()` 后使用 Node.js 文件 API 每次重新打开 `X_SITES_FILE` 指定的绝对路径；数据无静态导入、进程缓存或 ISR。字段校验的纯模块同时供页面和镜像中的只读校验命令复用，不增加 HTTP 数据接口、数据库模型或 OpenAPI 契约。

生产 Web 将 `X_DATA_DIR` 指定的宿主机目录只读挂载到 `/app/runtime-data`，固定读取 `x-sites.json`。挂载源必须预先存在；镜像只交付程序，模板单独初始化且不覆盖已有数据。维护者在同目录原子替换文件，后续请求重新打开新文件；禁止替换挂载目录本身。

JSON 必须为站点数组，包含字符串名称、HTTP/HTTPS 网址与简介；名称非空，URL 按标准化结果去重，展示值和顺序保持原样。空数组显示暂无站点；配置、读取、语法或字段失败记录脱敏错误类别及位置，并抛出安全异常，由 `/x` 局部中文错误边界提示完整重新加载。修复文件后重新请求恢复，不使用模板、旧内容或空列表掩盖错误。

本页面沿用现有公开可访问、无首页入口、`noindex, nofollow` 的边界，不使用管理员查阅会话，也不存放帐号密码。动态页面响应不缓存，代理与 CDN 不得强制缓存 `/x`。已打开页面不轮询更新。具体配置、校验、权限和更新操作以[JSON 维护手册](../operations/x-navigation-json.md)为准。
