...
- 手动 `CI - Full Validation` 默认以 `full` 模式并行执行 Backend pytest 和 Admin/Web Vitest、生产构建，最后消费同一 Run 的产物执行 Web standalone、Admin 生产 Nginx dist E2E，并输出 v2 成功证据。低风险配置、文档或部署接线核验可以选择 `smoke` 模式；该模式跳过 Admin/Web Vitest 和 coverage，仍执行 Backend pytest、两端生产构建及 Chromium E2E，并输出独立的 smoke Artifact。smoke Artifact 不能作为 `strict` 源码交接的 Full Validation 证据。当前单维护者、1Panel 人工部署不要求再运行候选镜像 E2E，使用 CNB/TCR 来源、digest 与扫描核验及部署后健康和关键业务检查，步骤见[端到端人工发布手册](../operations/github-cnb-tcr-1panel-release-runbook.md)。Full Validation 验证的构建产物与最终 TCR 镜像仍有边界，人工核验不能表述为最终镜像组合已通过自动 E2E。
