...
- E2E 默认针对生产构建运行：Web 使用 `next build` 后的 standalone server，Admin 使用 `max build` 后的 dist，由 `scripts/e2e/admin-preview.mjs` 使用固定 Nginx 镜像及生产配置托管在 3001 端口。Web 构建脚本同时把静态资源准备到 standalone 目录；本地 Admin 日常开发继续使用原 Umi 包装器。
- Windows 和 CI 统一通过 `scripts/e2e/run-e2e.mjs` 启动并回收本次拥有的 Web 进程和 Admin Nginx 容器，再调用 Playwright。3000/3001 端口已占用时明确失败，不复用未知服务；退出时只清理本次启动的资源。
...
- Playwright 不由日常开发、`$git-sync`、Push、Pull Request 或定时任务自动运行。需要本地标准 Chromium E2E 时由用户明确授权；需要干净 Ubuntu 环境时由用户人工触发 GitHub 完整验证。Firefox 与 WebKit 也只在用户明确要求或派生项目验收计划明确授权时执行。单独的本地浏览器结果不能满足严格源码交接门禁；GitHub `full` 验证只有在 pytest、Vitest、production build 和 Chromium Playwright 全部成功并生成同 SHA Artifact 后，才形成发布可核验的重型验证证据。`smoke` 验证跳过 Admin/Web Vitest 和 coverage，保留 Web/Admin 桌面与移动端质量基线，并将 Web/Admin Stage C 认证和权限旅程限制在桌面端；它仍证明 Backend pytest、两端生产构建和缩减范围的 Chromium Playwright 通过，不能替代完整验证或生成可供 `strict` 交接核验的 v2 Artifact。
...
- 手动 `CI - Full Validation` 默认以 `full` 模式并行执行 Backend pytest 和 Admin/Web Vitest、生产构建，最后消费同一 Run 的产物执行 Web standalone、Admin 生产 Nginx dist E2E，并输出 v2 成功证据。低风险配置、文档或部署接线核验可以选择 `smoke` 模式；该模式跳过 Admin/Web Vitest 和 coverage，继续执行 Web/Admin 桌面与移动端质量基线，并只在桌面端执行 Stage C 认证和权限旅程，之后输出独立的 smoke Artifact。smoke Artifact 不能作为 `strict` 源码交接的 Full Validation 证据。当前单维护者、1Panel 人工部署不要求再运行候选镜像 E2E，使用 CNB/TCR 来源、digest 与扫描核验及部署后健康和关键业务检查，步骤见[端到端人工发布手册](../operations/github-cnb-tcr-1panel-release-runbook.md)。Full Validation 验证的构建产物与最终 TCR 镜像仍有边界，人工核验不能表述为最终镜像组合已通过自动 E2E。
