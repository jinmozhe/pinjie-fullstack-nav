...
| 文件                                               | 说明                                                             |
| -------------------------------------------------- | ---------------------------------------------------------------- |
| [PROJECT_REQUIREMENTS.md](PROJECT_REQUIREMENTS.md) | Nav 项目身份、继承基线、当前需求边界、非目标和阶段验收标准 |
...
| [0010-浏览器认证会话RBAC与审计决策.md](adr/0010-浏览器认证会话RBAC与审计决策.md) | 确定 Browser Cookie Profile、C/B 会话隔离、Refresh 权威、规范化 RBAC 与审计边界；删除确认范围由 ADR 0017 局部替代 |
...
| [0016-管理员身份与导航只读会话决策.md](adr/0016-管理员身份与导航只读会话决策.md) | 管理员身份复用、独立查阅会话、两端退出与明文凭据边界 |
| [0017-Admin删除操作统一确认决策.md](adr/0017-Admin删除操作统一确认决策.md) | 管理端单条与批量软硬删除统一确认，保留业务原因和目标快照 |

---

## architecture/ - 架构设计说明

导航专项：[导航管理与管理员查阅](architecture/navigation.md)。
...
> 本目录继承自母版，保存业务扩展的参考设计；其中内容不自动成为 Nav 项目需求，采用前必须进入当前 PRD 和已确认计划。
...
导航专项：[导航功能启用](operations/navigation-setup.md)。
...
| [github-cnb-tcr-1panel-release-runbook.md](operations/github-cnb-tcr-1panel-release-runbook.md) | CNB Token、GitHub Secret、可选本机 TCR 核验与远程缓存排障入口，以及 GitHub Actions 经 CNB、TCR 到 1Panel 的发布、验证和回滚手册 |
...
| [tencent-tcr-personal-cam-accounts.md](operations/tencent-tcr-personal-cam-accounts.md) | TCR 个人版身份隔离、CAM 三仓最小权限、凭据初始化、Windows 本机与服务器登录、镜像查询、轮换和排障步骤 |
| [1panel-production-runbook.md](operations/1panel-production-runbook.md) | 1Panel 单机生产配置、迁移、OpenResty、日志、备份、恢复和回滚步骤 |
| [x-navigation-json.md](operations/x-navigation-json.md) | `/x` JSON 的本地初始化、生产挂载与权限、文件校验、原子更新及恢复步骤 |
