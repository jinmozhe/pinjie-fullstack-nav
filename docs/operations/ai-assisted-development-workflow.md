...
本文面向使用 AI 助手维护 `pinjie-fullstack-nav` 的项目负责人、开发者和评审者，完整说明：
...
- 涉及 Admin 列表页面时，按 [Admin 工程标准](../architecture/admin-engineering-standard.md)的“列表工具栏与查询”核对左侧“XX列表”标题、右侧刷新和业务操作；开发前评估查询需求，预计数据较多且需求不明确时向用户确认搜索、筛选及字段，避免每个列表机械增加查询区域。
