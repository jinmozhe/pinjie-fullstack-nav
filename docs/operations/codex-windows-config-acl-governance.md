...
- 母版历史计划《Codex Windows ACL 长期治理计划》保存 2026-08-22 本机 A/B 验证和历史实施证据。
- 母版历史计划《Codex Windows 配置与 ACL 标准文档计划》保存本文建立、确认、实施和验证记录。
- 母版历史计划《Codex Windows 网络与 Schannel 边界治理计划》保存默认联网决策、系统 HTTPS 客户端边界和同步记录。
- 母版历史计划《Codex Windows gh Keyring 宿主执行治理计划》保存 GitHub CLI 凭据上下文、准确宿主执行、审批规则和安全边界的实施记录。

上述母版计划已从当前工作树清理，可通过 `pinjie-fullstack-base` `v1.0.0` 与 Git 历史追溯。
