...
5. 管理端在“分类管理”（`/categories`）和“标签管理”（`/tags`）独立页面创建分类及可选标签，再进入“站点管理”（`/navigation`）创建站点、图标和帐号并发布；编辑站点时可打开“置顶”开关。侧栏三个入口依次位于“欢迎”下方，站点管理页内保留“站点”和“回收站”视图。
6. 访问 Web 首页，检查公开站点、搜索、分类和标签；访问 `/top` 检查置顶站点及空态；管理员登录后检查帐号与密码原样显示。
