...
1. 确认备份可恢复，并核对目标数据库。
2. 在 `apps/backend` 运行 `uv run alembic upgrade head`，导航迁移依次为 `20260906_01`、`20260906_02`、`20260907_01`、`20260907_02`、`20260908_01` 和 `20260911_01`。`20260907_01` 给分类增加非空 `requires_login`，已有分类默认公开；`20260907_02` 增加可空 `icon_key` 和允许值检查约束，已有分类保持 null 并显示默认图标；`20260911_01` 将站点 `category_id` 改为可空，既有数据不变。在启用新后端前完成升级并核对字段，不修改原身份表。
...
5. 管理端在“分类管理”（`/categories`）和“标签管理”（`/tags`）独立页面创建分类及可选标签，再进入“站点管理”（`/navigation`）创建站点、图标和帐号并发布；可先保存未分类站点，补充分类后才能发布或置顶。侧栏三个入口依次位于“欢迎”下方，站点管理页内保留“站点”和“回收站”视图。
