...
- Admin 与 Web 站点列表统一按 `sort_order` 升序、同值站点 ID 倒序；新建站点默认排序值调整为 `1,000,000`，历史站点数据不迁移。同步更新 OpenAPI、生成客户端和回归断言；轻量门禁通过，Backend 编译与应用导入受既有语法错误阻断，重型验证未执行。

- Web `/x` 改为每次请求读取服务端 JSON，原有 40 个站点原值与顺序迁移至初始化模板；增加字段与 URL 校验、中文失败重载和空态。生产 Web 只读挂载独立数据目录，提供同源 JSON 校验命令及初始化、原子更新、备份恢复步骤。运行数据不进入 Git 或镜像；轻量检查通过，production build、Vitest、浏览器验收、镜像发布与部署未执行。

- Web 首页和 Top 页改为按需管理员登录，取消加载与刷新时的静默授权跳转；已有有效查阅会话继续可用。Admin 授权页身份检查不再自动刷新，未登录直接进入原登录页并保留授权返回路径。同步回归用例源码与需求、机制文档；重型验证、镜像发布和部署未执行。
...
- Admin 站点、分类、标签管理统一复用用户管理的 PageFrame 容器，修复移动端页面及列表左右留白过大；沿用 16px 页面与卡片内边距、公共表格滚动和原有工具栏功能。

- Admin 站点搜索按用户要求移除简介匹配，仅匹配名称或域名；修复站点及回收站工具栏窄屏布局，分类与标签等分宽度，搜索随剩余空间伸缩，保留桌面 180px 搜索框和 Ant Design 组件。

- Admin 与 Web 站点搜索增加域名包含匹配，忽略英文大小写，保留 Admin 简介搜索及两端既有筛选、分页和可见范围；域名匹配只读取 URL 主机部分，两端搜索提示同步更新，无数据库迁移。

- Admin 站点列表及回收站增加分类、标签筛选，可搜索选项、单独或组合查询并清空条件；复用现有后端筛选与分页总数，保留关键词搜索，筛选变化时重置页码及勾选。

- Web 帐号查阅包含启用与停用资料，按启用优先、同状态最近更新优先排列并标记状态，保留授权和复制能力；Admin 帐号列表使用 Ant Design 状态开关，沿用凭据写权限与批量启停接口。无需数据库迁移，重型测试与浏览器验收未执行。

- Admin 分类管理与标签管理的“状态”列改用 Ant Design 开关，直接启用或停用，提供逐行加载、失败反馈及维护权限控制，保留原有批量操作。

- Admin 站点列表“置顶”列改用 Ant Design 开关，支持直接开启与关闭、逐行保存反馈和写权限控制；无维护权限及回收站保持只读，沿用现有站点保存接口。

- 增加独立站点置顶能力：Admin 站点表单维护置顶状态，Web `/top` 按当前身份可见范围汇总站点并分页；首页分组、排序和筛选逻辑保持不变。新增 `20260908_01` 迁移、OpenAPI 客户端字段及回归测试源码；实际迁移、重型测试和浏览器验收未执行。

- 完善分类与标签交叉筛选：按当前身份查询有关联站点的选项，补齐管理员分类内标签入口、非法参数与不可见目标错误、前端加载失败重试和清除入口；导航工具栏、搜索焦点和侧栏标签样式同步调整，身份轮询保留列表并继续隐藏重新核验中的凭据。OpenAPI 和客户端随实现重新生成；保留现有 Python 锁文件八项版本更新，重型验证未执行。

- 移除 Web 继承的字母 P 浏览器图标生成入口 `icon.tsx` 及根布局将后台 Logo 用作浏览器图标的声明，浏览器图标统一由用户放入的静态 `favicon.ico` 提供；页面品牌和收录站点图标不受影响。生产构建与浏览器验收未执行。

- 交付“拾径导航”品牌名称、站点文案、原创 SVG Logo、透明 PNG、反白版本、横版组合与多尺寸图标素材，补充后台配置说明。素材独立于运行配置，未修改生产站点资料或应用图标路由，未执行部署。

- 完成本地 Chrome 收藏整理导入：对 1,032 条收藏进行网址与同站去重、访问检测及内容核对，在本机开发库新增 23 个一级分类、132 个标签和 484 个站点，每站 3 至 5 个标签；478 个站点公开、6 个项目系统仅管理员查阅可见。数据库与公开查询用例回读通过，保留本地排除清单及导入回执；浏览器收藏不变，不涉及生产数据、应用源码或契约变化。

- 管理端站点表单增加网址抓取：自动填入名称、简介与图标草稿，支持修改、缺失提示和失败重试，保存时复用资产上传；Backend 增加权限与 CSRF 保护的元数据端点，公网 DNS 与重定向校验、固定 IP/TLS 主机验证、有界请求和位图转换。同步生成契约与安全、表单回归测试源码；无数据库迁移，重型测试和浏览器验收未执行。

- 实现 Web 导航桌面与手机设计：分组首页、分类和标签列表、独立名称包含搜索、卡片详情与右上外链、居中版权；详情支持单行网址、通栏描述、响应式分类标签排列及授权帐号展开复制。Backend 增加有界分组和单站点资料读取，保持公开与管理员可见范围隔离，同步生成契约和回归测试源码。本地轻量检查通过；本次无迁移，构建、自动化测试和浏览器验收未执行。

- 增加站点回收站单条及批量永久删除：独立 `navigation:purge` 权限、标准确认和专用 API，事务校验全部目标后实际删除站点并级联删除帐号资料和标签关联，保留分类、标签、图标资产与审计；同步生成契约、缓存清理、分页修正及回归测试源码。本次无新增迁移，真实删除、权限同步和重型验证未执行。

- 增加分类内置图标配置：Admin 可搜索选择、预览及清除 20 个常用图标，分类列表与 Web 分类导航按同一语义标识展示；Backend 增加可空 `icon_key`、允许值校验和 `20260907_02` 迁移，同步 OpenAPI、生成客户端及回归测试源码。实际数据库迁移、重型测试与浏览器验收未执行。

- 按 Admin 列表工具栏规则补齐现有导航站点、分类、标签、帐号及安全日志表格的“XX列表”标题和刷新入口；保留原有权限控制与按需搜索配置。

- 增加分类“仅登录后可见”配置：Admin 分类表单和列表支持可见范围，Backend 公开查询统一过滤分类及所属站点、搜索和分页总数；新增管理员查阅查询入口，Web SSR 与内存缓存按查阅身份隔离，退出和失效清理受限内容及在途请求。新增 `20260907_01` 迁移与回归测试源码；实际数据库迁移和重型测试未执行。

- 加固 Nav 管理员回调地址构造：精确匹配后端配置，校验协议和固定路径，返回字段仅进入编码后的 fragment；补充回归测试源码，未运行 Vitest。启用 Nav 依赖图和 13 项轻量必需检查，并配置无 bypass 的 rebase 自动合并流程。

- 选择性吸收母版累计通用修复：管理员会话错误传播与权限刷新、头像资产一致性、Admin 安全响应头、生产 E2E 服务归属、结构化扫描证据和构建依赖层；保留 Nav 业务与 reader 会话，排除绑定母版发布身份的候选镜像工具。三端轻量门禁及契约零漂移检查通过；production build、Vitest、pytest、Playwright、数据库迁移、云构建和部署未执行，完整母版追溯见[同步记录](plans/2026-09-06_母版90644f5累计更新计划.md)。
...
- 修复 Admin 空表布局：公共表头与单元格不换行规则覆盖导航页和帐号抽屉，空表按容器宽度自动计算列宽，避免 `max-content` 与固定布局在无数据时造成异常横向撑开；统一隐藏辅助测量行，保留 ProTable 内建空态、操作列和表格内部滚动。浏览器视觉验收未执行。
...
- 更新 GitSync 授权与恢复规则：显式执行即处理本仓库全部本地分支、提交及可交付工作区改动，取消主分支领先或分叉时重复确认；按依赖保留整合，同一错误三次修复仍失败即中止并报告，由用户决策后续处理。Security 扫描仅由 GitHub Actions 执行，取消本地扫描及统一 pnpm 前置入口方案，各技术栈保留既有轻量验证。同步个人技能和开发指南，保留恢复、平台审批与真实阻塞边界。

- 增加 Admin 列表工具栏规则：所有列表页面的表格上方左侧显示“XX列表”，右侧提供刷新及按权限显示的业务操作；搜索、筛选与重置按需评估，需求不明确时在开发前确认。规则、工程标准和开发流程已同步，本项仅记录文档约束，现有页面未据此批量改造或验收。

- Admin 软删除、硬删除及已保存 LOGO 移除统一使用二次确认，补齐站点单条与批量回收站入口，用户删除保留可选原因；提交期间防重复与关闭、失败保留目标并反馈，LOGO 按打开弹窗时的 revision 删除。站点列顺序调整为 LOGO、站点、分类、网址、标签、排序、状态、操作，网址与标签继续弹性省略。同步确认规则与 ADR 0017，动态验证未执行。

- Admin 站点管理和回收站在站点名称前增加 LOGO 列，以固定 40px 方形区域等比显示现有图标，缺失或加载失败时显示默认图标；名称后增加网址列。网址和标签平分剩余宽度并单行省略，其余列固定宽度，操作列按可见按钮数量预留空间；网址悬停显示完整地址并支持打开站点，窄屏由表格内部横向滚动。

- 调整 Admin 导航入口：站点管理（原导航管理）移至欢迎下方，分类管理和标签管理拆为 `/categories`、`/tags` 两个同级独立页面并依次排列，避免共用站点路径前缀导致菜单同时选中，菜单与页面标题一致；站点管理保留站点与回收站，沿用原组件、批量操作和权限。
...
- 统一 Admin 管理操作确认规则（历史记录，其中“仅硬删除确认、软删除直接提交”已由 [ADR 0017](docs/adr/0017-Admin删除操作统一确认决策.md)替代）：整个管理端移除密码二次确认 Token 的授权语义、确认请求头和密码确认弹窗；当时只有角色、文件资产等物理硬删除在单条和批量入口共用标准警告弹窗，按钮固定为“确定”和“取消”，用户软删除、启停、凭据重置、会话撤销、身份与权限调整直接提交；Backend 继续执行管理员会话、资源权限、CSRF、事务内资源校验和审计。旧版确认端点按 ADR 0007 临时保留至 2026-09-26，仅用于契约迁移并记录弃用调用。
