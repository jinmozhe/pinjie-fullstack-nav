...
- 本仓库是从 `pinjie-fullstack-base` 派生的独立导航站全栈项目，技术栈为 FastAPI、Next.js、React、pnpm 和 Turborepo。
- 本项目继承认证、用户、管理、系统等通用能力，并在当前仓库内增加导航站业务。导航业务范围以 `docs/PROJECT_REQUIREMENTS.md` 和已确认计划为准；可复用能力是否回馈母版必须另行评估，不建立自动同步或运行时依赖。
...
- `docs/PROJECT_REQUIREMENTS.md` 是本项目身份、继承技术基线、导航业务需求、非目标和验收边界的权威来源。
- 新增、修改或取消产品能力前必须读取产品需求基线，并在全栈计划中优先列出关联的 `NAV-*` 需求编号；涉及继承技术基线时同时列出适用的 `BASE-*` 编号。
...
- 2026-09-05，用户已人工完成母版继承计划的一次性清理，AI 已按当前文件事实重建 `plans/INDEX.md`。该初始化例外已经结束，不得再次用于删除本项目计划。
- 本项目母版 Tag、对应完整 Commit SHA 和实际派生源码快照记录在 `PROJECT_INDEX.md`。当前及后续新增计划全部适用永久保护，完整边界见 `plans/README.md` 和 ADR 0015。
